...
function amount($amount,$type = 1,$length = 0){
    if($type == 2) return round($amount,8);
    else return round($amount,$length);  
}



function chargeCalc($amount)
{
    $gs = Generalsetting::first();
    return  $gs->transfer_fix_charge + ($amount * ($gs->transfer_percent_charge/100));
}
...
function amountConv($amount,$currency){
    return round($amount/$currency->rate,3);
}
...
function apiCall($provider,$action,$order = []){
    return Http::get($provider->api_url, [
