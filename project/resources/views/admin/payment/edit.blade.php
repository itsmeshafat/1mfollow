...
@section('title')
   @langg('Edit Gateway : '.$paymentgateway->name)
@endsection

@section('breadcrumb')
 <section class="section">
    <div class="section-header justify-content-between">
        <h1> @langg('Edit Gateway : '.$paymentgateway->name)</h1>
        <a href="{{route('admin.gateway')}}" class="btn btn-primary"><i class="fas fa-backward"></i> @langg('Back')</a>
    </div>
</section>
@endsection
              ...
              <div class="form-group row mb-3">
                  ...
                  <div class="form-group row mb-3">
                     ...
                     <div class="col-sm-8">
                       <select name="status" class="form-control">
                          <option value="1" {{$paymentgateway->status == 1 ? 'selected':''}}>@langg('Active')</option>
                          <option value="0" {{$paymentgateway->status == 0 ? 'selected':''}}>@langg('Inactive')</option>
                       </select>
                     </div>
                   </div>
               ...
               <div class="col-md-12">
                  <div class="form-group">
                     <label>@langg('Select Currency')</label>
                     <select name="currency_id[]" class="form-control currency">
                        @foreach(DB::table('currencies')->get() as $dcurr)
                        <option value="{{$dcurr->id}}" {{in_array($dcurr->id,$paymentgateway->currency_id) ? 'selected':''}} data-code="{{$dcurr->code}}">{{$dcurr->code}}</option>
                        @endforeach
                     </select>
                  </div>
               </div>
                 ...
                 <div class="form-group col-md-6">
                     ...
                     <div class="input-group">
                        ...
                        <div class="input-group-append">
                           <span class="input-group-text currency-code" id="my-addon">{{$gs->curr_code}}</span>
                        </div>
                     </div>
                 </div>
...
@push('script')
    <script>
      'use strict';
      var currCode = $('.currency').find(':selected').data('code')
      $('.currency-code').text(currCode);
      $('.currency').on('change',function () { 
         var curr = $(this).find(':selected').data('code');
         $('.currency-code').text(curr);
      });
